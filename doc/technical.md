...
`SoftBoxStage` 在盒体局部坐标内维护 7 组位置、速度、半径与撞击形变量。每帧把世界向下方向转换为盒体局部重力，执行边界反射和球体互斥；拖动直接改变盒体姿态，轻点为所有声音体注入冲量。盒壳使用冷灰蓝 MeshPhysicalMaterial，以实体透明度、透射、厚度和吸收距离共同塑造偏实体的半透明玻璃。七颗球按索引固定为七个声部；只有 `AudioEngine.hit()` 真正接受的碰撞才会同步驱动该球压扁、发光、扩散光晕、盒体脉冲和 DOM 声部指示器，避免无声的装饰闪动。
